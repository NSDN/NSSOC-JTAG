...
unsigned char  __code Prod_Des[] =								//产品字符串描述符
{
	...
	'N', 0x00, 'S', 0x00, 'S', 0x00, 'O', 0x00, 'C', 0x00, ' ', 0x00,
	'D', 0x00, 'e', 0x00, 'b', 0x00, 'u', 0x00, 'g', 0x00, 'g', 0x00,
	'e', 0x00, 'r', 0x00
};
unsigned char  __code Jtag_Des[] =								//产品字符串描述符
{
	...
	'N', 0x00, 'S', 0x00, 'S', 0x00, 'O', 0x00, 'C', 0x00, ' ', 0x00,
	'J', 0x00, 'T', 0x00, 'A', 0x00, 'G', 0x00
};
unsigned char  __code Manuf_Des[] =
{
	sizeof(Manuf_Des), 0x03,
	'N', 0x00, 'S', 0x00, 'D', 0x00, 'N', 0x00, ' ', 0x00, '/', 0x00,
	' ', 0x00, 'N', 0x00, 'S', 0x00, 'C', 0x00, 'A', 0x00, 'M', 0x00
};
...
{
    mTimer_x_SetData(0,1333);                                                  //非自动重载方式需重新给TH0和TL0赋值,1.333MHz/1333=1000Hz, 1ms
    SOF_Count ++;
	if(Modem_Count)
		Modem_Count --;
    if(Modem_Count == 1)
...
void init_timer() {
    mTimer0Clk12DivFsys();	                                                   //T0定时器时钟设置,16MHz/12=1.333MHz
    mTimer_x_ModInit(0,1);                                                     //T0 定时器模式设置
    mTimer_x_SetData(0,13333);	                                               //T0定时器赋值,1.333MHz/1333=1000Hz, 1ms
